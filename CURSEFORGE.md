# Publishing HotsNDots on CurseForge
...
- **Project URL / slug:** hotsndots (if available)
...
- The zip must contain exactly one top-level folder `HotsNDots/` with the
  `.toc` + `.lua` files inside.
- The ready file is on your Desktop: **`HotsNDots.zip`**.
...
- In `HotsNDots.toc` enable and fill: `## X-Curse-Project-ID: 123456`

---

## Route B — GitHub + automatic packager (for ongoing updates)

1. Create a GitHub repo and push the **contents** of the `HotsNDots` folder to
   the repo root (so `HotsNDots.toc`, `Core.lua`, ... sit at the top). The
...
- [ ] Zip contains exactly the top-level `HotsNDots/` folder
